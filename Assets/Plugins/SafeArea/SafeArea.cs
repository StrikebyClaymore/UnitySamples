...
namespace Crystal
{
    ...
    public class SafeArea : MonoBehaviour
    {
        ...
        protected Vector2Int LastScreenSize = new Vector2Int(0, 0);
        protected ScreenOrientation LastOrientation = ScreenOrientation.AutoRotation;
        protected CanvasScaler CanvasScaler;
        protected Vector2 InitialReferenceResolution;

        [SerializeField] RectTransform TopPanel;
        [SerializeField] RectTransform CenterPanel;
        [SerializeField] RectTransform BottonPanel;
        ...
        [SerializeField] protected bool AdjustCanvasScaler = true;
        ...
        {
            AdjustSafeAreaCallback = safeAreaAdjuster;
        }
        ...
        private Rect GetSafeArea()
        {
            ...
            if (AdjustSafeAreaCallback != null)
            {
                safeArea = AdjustSafeAreaCallback(safeArea);
            }
        ...
        protected virtual void ApplySafeArea(Rect r)
        {
            ...
            if (Screen.width > 0 && Screen.height > 0)
            {
                ...
                {
                    CenterPanel.anchorMin = anchorMin;
                    CenterPanel.anchorMax = anchorMax;

                    if (TopPanel != null)
                    {
                        TopPanel.anchorMin = new Vector2(0, anchorMax.y);
                    ...
                    if (BottonPanel != null)
                    {
                        BottonPanel.anchorMin = new Vector2(0, 0);
                        BottonPanel.anchorMax = new Vector2(1, anchorMin.y);
                        BottonPanel.anchoredPosition = Vector2.zero;
                    }

                    if (AdjustCanvasScaler && CanvasScaler != null)
                    {
                        CanvasScaler.referenceResolution = new Vector2(
                            InitialReferenceResolution.x / (Panel.anchorMax.x - Panel.anchorMin.x),
                            InitialReferenceResolution.y / (Panel.anchorMax.y - Panel.anchorMin.y));
                    }
                }
            }
